...
public:
	...
	struct Mosi { static void connect();
		static_assert(
			...
			"GpioB8::Mosi only connects to Spi5!");
	};
	template< Peripheral peripheral >
	...
	struct Sd { static void connect();
		static_assert(
			(peripheral == Peripheral::I2s5),
			"GpioB8::Sd only connects to I2s5!");
	};
	template< Peripheral peripheral >
	struct Sda { static void connect();
		static_assert(
			(peripheral == Peripheral::I2c3),
			"GpioB8::Sda only connects to I2c3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioB8::Mosi<Peripheral::Spi5>
{
	using Gpio = GpioB8;
...
template<>
struct GpioB8::Sd<Peripheral::I2s5>
{
	using Gpio = GpioB8;
...
template<>
struct GpioB8::Sda<Peripheral::I2c3>
{
	using Gpio = GpioB8;
