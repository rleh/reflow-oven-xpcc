...
public:
	...
	struct Ck { static void connect();
		static_assert(
			...
			"GpioB12::Ck only connects to I2s3!");
	};
	template< Peripheral peripheral >
	struct Nss { static void connect();
		static_assert(
			...
			"GpioB12::Nss only connects to Spi2 or Spi4!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi3),
			"GpioB12::Sck only connects to Spi3!");
	};
	template< Peripheral peripheral >
	...
	struct Ws { static void connect();
		static_assert(
			...
			(peripheral == Peripheral::I2s4),
			"GpioB12::Ws only connects to I2s2 or I2s4!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioB12::Ck<Peripheral::I2s3>
{
	using Gpio = GpioB12;
...
template<>
struct GpioB12::Nss<Peripheral::Spi4>
{
	using Gpio = GpioB12;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Nss;
...
template<>
struct GpioB12::Sck<Peripheral::Spi3>
{
	using Gpio = GpioB12;
...
template<>
struct GpioB12::Ws<Peripheral::I2s4>
{
	using Gpio = GpioB12;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Ws;
	static constexpr int af = 6;
